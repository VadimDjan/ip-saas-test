describe('Автотест на получение Титула. ', function () {
    ...
    it('12 Выходим из системы', function (done) {
...
// 9) Перейти во вкладку "Технологические окна", нажать кнопку "Добавить запись"
// 10) в открывшемся окне заполнить поля "Длительность, часов", "Начало" и нажать кнопку Создать и Сохранить. Убеждаемся, что сохранение произошло.
// Справа наверху возникло зеленое сообщение. Нажимаем кнопку Назад, проверить что в списке есть хотя бы 1 значение
// 11) Вернуться к задаче в Моих нарядах (отфильстровать и открыть нужное по Id) и нажать на кнопку Выполнить. Убедиться, что значение поля статус изменилось. Нажать кнопку "Назад"
