describe("Автотест распределение по ПМС.",
  function () {
    ...
    it("1. Переходим по URL /#/my_tasks_wc_for_test. Ждём загрузки и убеждаемся, что отобразилась таблица и в ней есть хотя бы одна запись.  ##can_continue",
      async (done) => {
    ...
    it('2. Выбираем наряд "Распределить участки ремонта пути по ПМС" и открываем его. Убеждаемся, что запись открылась, в ней есть вкладки, поля и кнопка "Сохранить".  ##can_continue',
      async (done) => {
    ...
    it("3. В наряде указываем исполнителя и жмем на кнопку Сохранить. Убеждаемся, что сохранение произошло. Справа наверху возникло зеленое сообщение. ##can_continue",
      async (done) => {
    ...
    it("4. Жмем кнопку В работу.Убеждаемся, что значение поля Статус изменилось. ##can_continue",
      async (done) => {
    ...
    // 5. Переходим по ссылке "Перейти к ДПГ". Убеждаемся, что по клику открылась таблица, в ней есть кнопка "Добавить запись".
    it('5. Переходим по ссылке "Перейти к ДПГ". Убеждаемся, что по клику открылась таблица, в ней есть кнопка "+Добавить". ##can_continue',
      async (done) => {
        console.log('5. Переходим по ссылке "Перейти к ДПГ". Убеждаемся, что по клику открылась таблица, в ней есть кнопка "+Добавить".');
        await errorCatcher(async () => {
          await browser.sleep(1500);
    ...
    it('7. В открывшемся окне убедится что есть поле "ПМС", кнопки "ОК" и "Отмена" выбрать ПМС-197 из выпадающего списка, нажать "Ок". ##can_continue',
      async (done) => {
    ...
    it('8. Вернуться к задаче и нажать на кнопку Выполнить. Убедиться, что значение поля статус изменился на "Выполнен". ##can_continue',
      async (done) => {
