describe('Автотест на создание этапов и режима выполнения участков ', function () {
  ...
  $h.dpgAddWorkCompletionStageId = 3748656;
      ...
      await $h.grid.main.setSearch([
        {
          ...
          operator: 'eq',
          field: 'taskid',
          value: $h.dpgAddWorkCompletionStageId,
        }
      ]);
      await browser.sleep(2000);
      const webElement = await element.all(by.css('[data-pkfieldid=\"' + String($h.dpgAddWorkCompletionStageId) + '\"]')).first().getWebElement();
  ...
  it('5. Переходим по ссылке "К просмотру и созданию этапов ДПГ". Убеждаемся, что по клику открылась таблица, в ней есть кнопка "+Добавить" и нажимаем на неё ##can_continue', async done => {
    await errorCatcher(async () => {
      ...
      await browser.wait(EC.presenceOf(element(by.css('.toolbar-buttons.k-button.k-grid-add'))), 5000);
