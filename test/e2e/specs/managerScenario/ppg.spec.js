  ...
  function () {
    ...
    it("0. Заходим в систему под ПМС197_Менеджер. ##can_continue", async (done) => {
        await errorCatcher(async () => {
          ...
          await $h.login.loginToPage(
            null,
            loginObject.users[3].user,
            loginObject.users[3].password
          );
    ...
    const addWorkingPeriod = async () => {
      ...
      try {
        ...
        await element(by.css('div[data-field-name="working_periods"] .idea-button-add-row-modal')).click();
    ...
    const clickOnArrow = async () => {
      ...
      try {
        const arrow = await element.all(by.css(".ip-task-link a path")).filter(async (elem, index) => {
