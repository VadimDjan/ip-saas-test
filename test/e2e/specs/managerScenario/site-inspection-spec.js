describe("Автотест на Осмотр участка. ", function () {
    ...
    it("2. Выполняем каждый наряд на осмотр участка.", async (done) => {
      const selector =
        '.modal-body[data-detail="task"] .card .react-grid-item[data-field-name="workflowstepid"] input';
    ...
    const fillSiteLengthData = async (request) => {
      ...
      try {
        const form = await $h.form.getForm([
          "start_km_br",
          "start_pk_br",
          "finish_km",
          "finish_pk_before_repair",
    ...
    const fillRollingStock = async () => {
      ...
      try {
        ...
        for (let i = 0; i < 2 - count; i++) {
          const buttonElement = elementMain.$("a.idea-button-add-row-modal");
    ...
    const saveSiteInspection = async () => {
      ...
      try {
        ...
        await browser.wait(
          EC.presenceOf(
            element(by.cssContainingText(".linkfield__link", "Заполнить данные по осмотру участка"))), defaultWaitTimeout);
