describe('Автотест на Осмотр участка. ', function () {
    ...
    //     $h.login.loginToPage(null, loginObject.users[2].user, loginObject.users[2].password)
    //         .then(angularWait)
    //         .then(expliciteWait)
    //         .then(function () {
    ...
    it('00. Переходим по URL /#/service. ##can_continue', (done) => {
        protractor.helpers.serviceId = 808;
        return angularWait()
            .then(function () {
            ...
            .then(angularWait)
            .then(expliciteWait)
            .then(element(by.css('.toolbar-buttons.k-button.idea-button-clear-filters')).click())
            .then(angularWait)
            .then(expliciteWait)
            .then(done);
    }, skip);

    it('0. Закрываем модальное окно. Переходим на вкладку "Участки ремонта пути", выбираем первый участок и открываем его. Убеждаемся, что запись открылась, в ней есть вкладки, поля и кнопка "Сохранить".  ##can_continue', function (done) {
        ...
        //         });
        //     })
        //     .then(browser.wait(EC.presenceOf(element(by.css('.back-button'))), 30000))
        //     .then(function () {
        ...
        return $h.grid.main.setSearch([
            {
                ...
                operator: 'eq',
                field: 'serviceid',
                value:  protractor.helpers.serviceId,
            }
        ])
            .then(angularWait)
            .then(expliciteWait)
            ...
            .then(function () {
                element.all(by.css('[data-pkfieldid=\"' + String(protractor.helpers.serviceId) + '\"]')).first().getWebElement()
                    .then(function (event) {
    ...
    it('1. Закрываем модальное окно. Переходим на вкладку "Участки ремонта пути", выбираем первый участок и открываем его. Убеждаемся, что запись открылась, в ней есть вкладки, поля и кнопка "Сохранить".  ##can_continue', function (done) {
        // const selector = '.details__close-btn';
        // browser.sleep(1000)
        //     .then(function () {
        //         element.all(by.css(selector)).then( function(items) {
        ...
        //     })
        //     .then(browser.sleep(1500));
        return $h.form.getForm(['requests'])
            .then(function (form) {
                eventUid = form.uid;
                tasksBefore = form.requests.length;
                const index = form.requests[0].items.findIndex(item => {
                    return item.workflowstepid === '371$>$>$Построение\n' +
                        'часового графика'
                });
                protractor.helpers.requestsUid = Number(form.requests[0].items[index].requestid);
            })
            .then(function () {
                element.all(by.css('[data-pkfieldid=\"' + String(protractor.helpers.requestsUid) + '\"]')).first().getWebElement()
                    .then(function (event) {
                        browser.actions().doubleClick(event).perform();
                        return browser.sleep(1000);
                    })
            })
            .then(browser.wait(EC.presenceOf(element(by.css('[data-button-name="calc_mvsp"]'))), 5000))
            .then(function (res) {
                expect(element(by.css('[data-button-name="calc_mvsp"]')).getText()).toBe(buttonCalcMvsp);   // Проверить что есть кнопка Сохранить
            })
            .then(done);
    }, skip)
    ...
    it('2. Кликаем по ссылке и убеждаемся что отобразилась таблица и в ней имеются не менее 3х записей. ##can_continue', function (done) {
        ...
        return  element(by.css('[class="glyphicon glyphicon-road"]')).click()
            ...
            .then(function () {
                ...
                expect(element(by.css('[data-button-name="request_cancel2"]')).getText()).toBe('Отменить участок');   // Проверить что есть кнопка "Отменить участок"
            ...
            .then(function () {
                expect(element(by.css('[data-field-name="railtrack_class"]')).isPresent()).toBe(true);   // Класс, группа, категория пути
                expect(element(by.css('[data-field-name="track_specialization"]')).isPresent()).toBe(true);   // Специализация жд линии
                expect(element(by.css('[data-field-name="temp_amplitude"]')).isPresent()).toBe(true);
                ;  // Проверить что есть поле "Темп. амплитуда"
                expect(element(by.css('[data-field-name="department"]')).isPresent()).toBe(true);   // Проверить что есть поле "ПМС"
            })
            .then(angularWait)
            .then(expliciteWait)
            .then(done);
    ...
    it('3. Заполнить обязательные поля на вкладке "Общая информация" . ##can_continue', function (done) {
