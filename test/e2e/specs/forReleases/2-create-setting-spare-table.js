describe("Регистрация пространства", function () {
  ...
  it("Добавить запись таблицу Запчасть", async (done) => {
    ...
    await errorCatcher(async () => {
      ...
      if (hasOneRow && hasRowList) await console.log("формы списка заказов и одной записи заказа созданы");
      await browser.sleep(2000);
      // open form designer
      ...
      await browser.sleep(2000);
      await element.all(by.css(".k-animation-container .k-numerictextbox span input")).first().sendKeys("10");
      await browser.sleep(200);
