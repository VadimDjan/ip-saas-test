describe('Workflow', function () {
    ...
    it('Add workflow for order_table', async done => {
        ...
        await errorCatcher(async () => {
            await browser.get('http://51.250.15.24:8080/#/workflow'); //http://localhost:8081
            ...
            await browser.wait(EC.presenceOf(element(by.cssContainingText('.form-header__title span', 'Work Flow'))), defaultWaitTimeout);
            await browser.sleep(2000);
    ...
    it('Create workflow steps for the order_table', async done => {
        ...
        await errorCatcher(async () => {
            ...
            await browser.sleep(2000);
            await element(by.css('div[data-field-name="internalname"] input')).sendKeys('to prepare');
            await browser.sleep(200);
            await element(by.css('button[data-button-name="CREATE"]')).click();
            await browser.sleep(2000);
            await element(by.css('.alerts__container .alert-count')).click();
            ...
            await browser.sleep(2000);
            await element(by.cssContainingText('.k-animation-container ul li', 'demo')).click();
            await browser.sleep(2000);
    ...
    it('Add workflow and create workflow steps for the order_punkt', async done => {
        ...
        await errorCatcher(async () => {
            // await browser.get('http://51.250.15.24:8080/#/workflow/');
    ...
    it('Set workflow to the order_table', async done => {
        ...
        await errorCatcher(async () => {
            ...
            await browser.sleep(2000);
            await element(by.cssContainingText('div[data-field-name="defaultworkflow"] div[role="combobox"] .rw-popup ul li span', 'test-wf')).click();
            await browser.sleep(2000);
            const code = `if (!CommonUtils.isEmpty(QueryUtils.select("address").from("order_table"))) { workflowIdOrName = ${workFlowId}; } else { workflowIdOrName = 106; }`;
    ...
    it('Add row to order_table table', async done => {
        await errorCatcher(async () => {
            // 1
            await browser.get('http://51.250.15.24:8080/#/order_table');
            await browser.sleep(2000);
