describe('Создание и настройка отчёта по менеджерам и дашборда менеджера', function() {
  ...
  it('Создание таблицы для отчёта Анализ работы менеджеров', async done => {
    console.log('Создание таблицы для отчёта Анализ работы менеджеров');
    await errorCatcher(async () => {
      await browser.get('http://51.250.15.24:8080/#/entitytype/');
      ...
      await browser.wait(EC.presenceOf(element(by.cssContainingText('div[data-field-name="entityfieldlist"] .subgrid-table-title', 'Поля'))), defaultWaitTimeout);
      await browser.sleep(200);
      // добавим поле Цвет
      ...
      await element(by.css('div[data-field-name="fieldformatid"] .comboboxfield__container')).click();
      await browser.sleep(2000);
      await element(by.cssContainingText('div[data-field-name="fieldformatid"] .rw-popup-container input')).sendKeys('Пользователь (User)');
      await browser.sleep(2000);
  ...
  it('Создание отчёта Анализ работы менеджеров', async done => {
    console.log('Создание отчёта Анализ работы менеджеров');
    await errorCatcher(async () => {
      ...
      await browser.sleep(2000);
      await element(by.css('div[data-field-name="entitytypeid"] .rw-popup-container .rw-popup input')).sendKeys('Анализ работы менеджеров');
      await browser.sleep(2000);
  ...
  it('Создание дочернего отчёта у Анализа по менеджерам', async done => {
    console.log('Создание дочернего отчёта у Анализа по менеджерам');
    await errorCatcher(async () => {
      await element(by.css('.idea-button-add-row-modal')).click();
      await browser.sleep(2000);
  ...
  it('Дашборд менеджера', async done => {
    ...
    await errorCatcher(async () => {
      await browser.get('http://51.250.15.24:8080/#/dashboardtype/');
      await browser.wait(EC.presenceOf(element(by.cssContainingText('.table-name__wrapper', 'DashboardType'))), defaultWaitTimeout);
      await browser.sleep(2000);
